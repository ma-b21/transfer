...
class Handler:
    ...
    def handle_receive(self, symbol_duration, sampling_rate, signal_freq):
        ...
        abs_signal = np.abs(self.signal)
        env_smoothed = np.convolve(abs_signal, kernel, mode='same')

        # ========== 3) 动态阈值：假设前面一小段是噪声底部，用均值估算噪声，再乘以系数 ==========
        noise_len = int(0.25 * sampling_rate)  # 前1秒当做纯噪声区
        noise_floor = np.mean(
            env_smoothed[sampling_rate: sampling_rate + noise_len])
        threshold_ratio = 5  # 阈值系数
        threshold = noise_floor * threshold_ratio

        # ========== 4) 根据阈值区分活跃段(> 阈值)和静音段(<= 阈值) ==========
        active_mask = (env_smoothed > threshold).astype(np.int16)

        # ========== 5) 根据最小静音长度分割包 ==========
        # 若检测到活跃 -> 静音，并且后续持续静音超过 min_silence_len，认为一个包结束
        min_silence_len = 0.25  # 0.25秒的静音判定包间隔
        min_silence_samples = int(sampling_rate * min_silence_len)
        ...
        for i in range(len(active_mask)):
            if not is_signal and active_mask[i] == 1:
                # 检测到信号起点
                is_signal = True
                start_idx = i
            elif is_signal and active_mask[i] == 0:
                # 可能进入静音，检查后续静音是否足够长
                if i + min_silence_samples < len(active_mask):
                    if i - start_idx < int(symbol_duration * sampling_rate) * 28:
                        continue
                    if np.sum(active_mask[i: i+min_silence_samples]) < min_silence_samples // 4:
                        # 确认真正到达包末尾
                        end_idx = i
                        package_signal.append(self.signal[max(0, start_idx - 2000):min(len(self.signal), end_idx + 2000)])
                        is_signal = False

        # 如果最后一段仍有活跃，则补充进来
        if is_signal:
            package_signal.append(self.signal[start_idx:])

        # ========== 6) 处理每个分段 ==========
        for idx, pkg in enumerate(package_signal):
            package_signal[idx] = handle_receive(
                pkg, symbol_duration, sampling_rate, signal_freq)
            if package_signal[idx] == "未检测到前导码" or package_signal[idx] == "头数据长度不足" or package_signal[idx] == "包数据长度不足":
                package_signal[idx] = ""
            print(f"Packet {idx+1} length: {len(pkg)} text: {package_signal[idx]}")

        print("Detected packets:", len(package_signal))
